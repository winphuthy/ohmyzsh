...
prompt_context() {
  if [[ -n "$SSH_CLIENT" ]]; then
    ...
    prompt_segment black magenta "%{$fg_bold[magenta]%(!.%{%F{magenta}%}.)%}@$USER%{$fg_no_bold[magenta]%}"
  fi
}
...
prompt_dir() {
  prompt_segment 26 white "%{$fg_bold[white]%}%~%{$fg_no_bold[white]%}"
}
...
prompt_time() {
  prompt_segment 63 white "%{$fg_bold[white]%}%D{%a %e %b - %H:%M}%{$fg_no_bold[white]%}"
}
